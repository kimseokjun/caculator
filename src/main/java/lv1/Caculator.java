...
public class Caculator {
    ...
    public static void main(String[] args) {
        ...
        lv2.Caculator c = new lv2.Caculator();  //lv2
        ...
        while (true) {
            ...
            switch (operation) {
                case '+':
                    c.plus(firstNum, secondNum);    //lv2
                    firstNum += secondNum;
                    break;
                case '-':
                    c.minus(firstNum, secondNum);     //lv2
                    firstNum -= secondNum;
                    break;
                case '/':
                    if (secondNum == 0) {
                    ...
                    c.div(firstNum, secondNum);   //lv2
                    firstNum /= secondNum;
                    break;
                case '*':
                    c.multi(firstNum, secondNum);     //lv2
                    firstNum *= secondNum;
                    break;
                default:
            ...
            System.out.println("get메서드를 통한 값 접근 : " + c.getResult());
            System.out.print("첫번째 데이터를 삭제하려면 del 입력 : ");
            String del = sc.next();
            if (del.equals("del")) {
                c.removeResult();
                System.out.println("삭제후 데이터 내역 : " + c.getResult());
            }

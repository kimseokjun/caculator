...
public class Caculator {
    ...
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            switch (operation) {
                case '+':
                    c.plus(firstNum, secondNum);    //lv2 요거 list에 담기
                    c.setResult(firstNum+secondNum);    //set을이용해서 lv2 Calculator result 접근
                    break;
                case '-':
                    c.minus(firstNum, secondNum);
                    c.setResult(firstNum-secondNum);
                    break;
                case '/':
                    if (secondNum == 0) {
            ...
            System.out.println("get메서드를 통한 값 접근(result) : " + c.getResult());
